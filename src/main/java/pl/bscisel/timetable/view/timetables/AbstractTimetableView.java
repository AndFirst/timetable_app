...
import pl.bscisel.timetable.view.timetables.components.MyFullCalendar;

import java.time.ZoneId;

// LLR_100
public abstract class AbstractTimetableView extends VerticalLayout {
    ...
    public AbstractTimetableView(EventsService eventsService) {
    ...
    private void addListenersForMovingAndResizing() {
        ...
        entryResizedListener = calendar.addEntryResizedListener((event) -> {
            ...
            TimetableEntry timetableEntry = (TimetableEntry) event.getEntry();
            Event dbEvent = timetableEntry.getEvent();
            timetableEntry.setRecurringEndTime(dbEvent.getEndTime().plusHours(event.getDelta().getHours()).plusMinutes(event.getDelta().getMinutes()));
        ...
        entryDroppedListener = calendar.addEntryDroppedListener(event -> {
            ...
            TimetableEntry timetableEntry = (TimetableEntry) event.getEntry();
            Event dbEvent = timetableEntry.getEvent();
            timetableEntry.setRecurringStartTime(dbEvent.getStartTime().plusHours(event.getDelta().getHours()).plusMinutes(event.getDelta().getMinutes()));
            timetableEntry.setRecurringEndTime(dbEvent.getEndTime().plusHours(event.getDelta().getHours()).plusMinutes(event.getDelta().getMinutes()));
            timetableEntry.setRecurringDaysOfWeek(dbEvent.getDayOfWeek().plus(event.getDelta().getDays()));
    ...
    private boolean isEntryNotEditable(Entry event) {
        return !(event instanceof TimetableEntry entry) || !entry.isEditable();
    }
